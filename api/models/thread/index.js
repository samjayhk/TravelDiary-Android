    ...
    return {
        list: (page) => {
            return new Promise((resolve, reject) => {
                ...
                connection.query('SELECT p.pid, p.uid, u.username, t.name AS tag, p.subject, p.ptime, p.uptime, p.rate, COUNT(c.pid) AS count, DATEDIFF(CURRENT_TIMESTAMP, p.ptime)  AS daylast, TIMEDIFF(CURRENT_TIMESTAMP, p.ptime) AS timelast FROM comment c RIGHT JOIN post p ON p.pid = c.pid INNER JOIN tags t ON p.tid = t.tid INNER JOIN users u ON u.uid = p.uid GROUP BY p.pid ORDER BY p.ptime DESC LIMIT ?, ?', [(parseInt(page)*8)-8, 8], (error, results) => {
                    if (results.length >= 1) {
                        if (parseInt(page) > 1) {
                            resolve({result: true, page: parseInt(page), thread: results})
                        } else {
        ...
        content: (pid, page) => {
            return new Promise((resolve, reject) => {
                const { connection, errorHandler } = deps
                connection.query('SELECT p.pid, p.uid, u.username, f.avatar, p.subject, p.content, p.ptime, p.uptime, COUNT(c.pid) AS count FROM post p, comment c, users u, usersinfo f WHERE p.pid = ? AND p.uid = u.uid AND p.uid = f.uid AND p.pid = c.pid ORDER BY p.ptime DESC', [parseInt(pid)], (error, thread_result) => {
                            ...
                            connection.query('SELECT c.cid, c.pid, c.uid, u.username, f.avatar, c.comment, c.ctime, c.uptime FROM comment c, users u, usersinfo f WHERE c.pid = ? AND c.uid = u.uid AND f.uid = c.uid ORDER BY c.cid ASC LIMIT ?, ?', [parseInt(pid), (parseInt(page)*8)-8, 8], (error, comment_result) => {
                                if (parseInt(page) > 1) {
                                    if (comment_result.length >= 1) {
                                        resolve({ result: true, page: parseInt(page), comment: comment_result})
                                    } else {
                                        resolve({ result: false, message: 'No others comment found.'})
                                    }
                                } else {
                                    resolve({ result: true, page: parseInt(page), sum: Math.ceil(parseInt(thread_result[0].count)/8), thread: thread_result, comment: comment_result})
                                }
                            })
                    } else {
